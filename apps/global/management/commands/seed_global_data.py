...
class Command(BaseCommand):
    help = "Seed database with initial data for Adikarya Lamongan"
    ...
    def _seed_company(self):
        ...
        CompanyProfile.objects.create(
            name='Adikarya',
            short_description='Toko komputer, laptop & printer di Lamongan. Melayani penjualan, servis, dan upgrade — jujur, bergaransi, dipercaya lebih dari 25 tahun.',
            about_text='<p>Berdiri sejak 1999, <strong>Adikarya</strong> dimulai dari toko kecil di Jl. Veteran Lamongan. Bermodalkan keahlian servis dan kejujuran, kami perlahan dipercaya oleh masyarakat Lamongan untuk menangani berbagai kebutuhan komputer — dari sekadar beli mouse hingga servis laptop mati total.</p><p>Satu prinsip yang tidak pernah berubah: <strong>kondisi perangkat disampaikan apa adanya, estimasi biaya sebelum dikerjakan, garansi tertulis untuk setiap servis.</strong> Itu sebabnya pelanggan kami — dari pelajar, mahasiswa, guru, hingga kantor-kantor di Lamongan — tetap setia sampai sekarang.</p><p>Saat ini, Adikarya terus berkembang. Stok kami semakin lengkap, teknisi kami terus belajar teknologi baru, dan toko kami tetap menjadi tempat yang ramah untuk sekadar bertanya seputar komputer.</p>',
            address='Jl. Veteran No. 10, Lamongan',
            phone='0821-4060-3875',
            whatsapp='6282140603875',
            email='zonakomputer.Lamongan@gmail.com',
            ...
            tiktok_url='',
            visi='Menjadi mitra teknologi nomor satu bagi masyarakat Lamongan dan sekitarnya — dari generasi ke generasi.',
    ...
    def _seed_stats(self):
        if CompanyStat.objects.exists():
            ...
            return
        stats = [
            ('Tahun Berdiri', '1999', 'Melayani Lamongan sejak 1999', '🏆', 1),
            ...
            ('Garansi Servis', '30 Hari',
             'Garansi tertulis untuk tiap pekerjaan', '🛡️', 5),
        ]
        for title, value, desc, icon, order in stats:
            CompanyStat.objects.create(
                title=title, value=value, description=desc, icon=icon, order=order)
    ...
    def _seed_team(self):
        if TeamMember.objects.exists():
            ...
            return
        members = [
            ('Hadi', 'Pemilik & Founder',
             'Servis Hardware, Manajemen Toko', '25+ tahun', 1),
            ('Arif', 'Teknisi Senior', 'Servis Laptop, PC, Printer', '15+ tahun', 2),
            ('Dimas', 'Teknisi', 'Servis Software, Jaringan, Upgrade', '8 tahun', 3),
            ('Sari', 'Admin & Customer Service',
             'Pelayanan pelanggan, stok & penjualan', '5 tahun', 4),
        ]
        for name, role, specialty, exp, order in members:
            TeamMember.objects.create(
                name=name, role=role, specialty=specialty, experience=exp, order=order)
    ...
    def _seed_faqs(self):
        ...
        faqs = [
            ...
            ('Apakah bisa konsultasi dulu sebelum servis?',
             'Tentu! Anda bisa chat via WhatsApp atau datang langsung ke toko untuk konsultasi gratis. Kami akan menjelaskan kerusakan, biaya, dan waktu pengerjaan sebelum memutuskan.', 4),
            ...
            ('Apakah ada layanan antar-jemput?',
             'Ya, untuk area Lamongan kota kami menyediakan layanan antar-jemput perangkat. Silakan hubungi kami untuk koordinasi lebih lanjut.', 7),
            ('Metode pembayaran apa saja?',
             'Kami menerima pembayaran Tunai, Transfer Bank (BCA, Mandiri, BRI), dan E-Wallet (GoPay, OVO, DANA, ShopeePay).', 8),
        ]
    ...
    def _seed_testimonials(self):
        if Testimonial.objects.exists():
            ...
            return
        testimonials = [
            ('Budi Santoso', 'Guru SMK', 'Laptop saya mati total dan data skripsi hampir hilang. Alhamdulillah data bisa diselamatkan dan laptop normal lagi. Makasih Adikarya!', 5, False, 1),
            ('Siti Nur Aini', 'Mahasiswa',
             'Pertama kali ke sini buat beli laptop, dijelasin sabar banget. Akhirnya dapet unit yang sesuai budget. Recomended!', 5, False, 2),
            ...
            ('Dinas Pendidikan', 'Instansi Pemerintah',
             'Kerja sama pengadaan PC kantor. Pengiriman tepat waktu, barang sesuai spek, garansi toko. Profesional.', 5, True, 4),
            ('Rudi Hartono', 'Teknisi IT', 'Tempat langganan beli sparepart dan komponen. Stok lumayan lengkap, harga bersaing. Recomended untuk teman-teman IT Lamongan.', 5, False, 5),
            ...
            ('SMAN 1 Lamongan', 'Sekolah',
             'Servis 20 unit PC laboratorium sekolah. Semua selesai tepat waktu dan sesuai budget. Terima kasih Adikarya.', 5, True, 7),
            ('Andi Prasetyo', 'Freelance Designer',
             'Upgrade RAM dan SSD di sini. Hasilnya laptop jadi ngebut banget buat desain. Teknisinya paham banget.', 5, True, 8),
        ]
    ...
    def _seed_promos(self):
        ...
        promos = [
            ...
            },
            {
                ...
                'category': 'Produk',
                'description': 'Beli laptop baru di Adikarya, dapatkan paket bundling hemat: tas laptop + mouse wireless + keyboard combo.',
    ...
    def _seed_gallery(self):
        ...
        items = [
            ('Tampak Depan Toko', 'Toko Adikarya di Jl. Veteran No. 10 Lamongan',
             '🏪', 'Toko & Suasana', 1),
            ('Ruang Servis', 'Area kerja teknisi dengan peralatan lengkap',
             '🔧', 'Servis & Perbaikan', 2),
            ('Rak Produk Laptop', 'Display laptop baru dan second siap jual',
             '💻', 'Produk & Barang', 3),
            ('Tim Adikarya', 'Foto bersama tim teknisi dan admin', '👥', 'Tim Kami', 4),
            ('Suasana Toko Pagi Hari', 'Toko siap melayani pelanggan',
             '☀️', 'Toko & Suasana', 5),
            ('Proses Servis Laptop', 'Teknisi sedang memperbaiki motherboard laptop',
             '⚡', 'Servis & Perbaikan', 6),
            ('Rak Sparepart & Komponen', 'Koleksi sparepart dan komponen siap pakai',
             '⚙️', 'Produk & Barang', 7),
            ('Sertifikat & Penghargaan',
             'Dokumentasi sertifikat workshop teknisi', '📜', 'Tim Kami', 8),
        ]
        for title, desc, icon, cat_name, order in items:
            cat = GalleryCategory.objects.get(name=cat_name)
            Gallery.objects.create(
                category=cat, title=title, description=desc, icon=icon, order=order)
        ...
        articles = [
            {
                ...
                'content': '<p>Laptop yang jarang dibersihkan bagian dalamnya bisa mengalami overheating, performa turun, bahkan kerusakan permanen pada komponen. Berikut 5 tanda laptop Anda perlu segera dibersihkan:</p><ol><li><strong>Kipas berbunyi kasar atau bising</strong> — debu yang menumpuk di kipas membuatnya tidak seimbang dan berisik.</li><li><strong>Suhu laptop panas berlebihan</strong> — ventilasi tersumbat debu membuat udara panas tidak bisa keluar.</li><li><strong>Performa menurun drastis</strong> — laptop sengaja menurunkan kecepatan prosesor (throttling) untuk mengurangi suhu.</li><li><strong>Sering mati mendadak</strong> — proteksi otomatis saat suhu sudah terlalu tinggi.</li><li><strong>Debu keluar dari ventilasi</strong> — tanda jelas bahwa bagian dalam sudah penuh debu.</li></ol><p>Bawa laptop Anda ke <strong>Adikarya Lamongan</strong> untuk pembersihan profesional. Kami buka casing, bersihkan kipas, heatsink, dan ganti thermal paste — laptop kembali dingin dan kencang.</p>',
                'icon': '🧹',
                'is_featured': True,
                'order': 1,
            },
            {
                ...
                'content': '<p>Memilih laptop untuk kebutuhan kuliah memang membingungkan — apalagi dengan banyaknya pilihan di pasaran. Berikut panduan praktis dari tim Adikarya:</p><p><strong>Budget 3-5 Juta:</strong> Cari laptop second berkualitas seperti ThinkPad atau EliteBook. Spek minimal: Intel i5 gen 8, 8GB RAM, 256GB SSD. Cukup untuk tugas, browsing, dan Microsoft Office.</p><p><strong>Budget 5-8 Juta:</strong> Bisa dapat laptop baru entry-level seperti ASUS VivoBook, Lenovo IdeaPad, atau Acer Aspire. Spek: i5/Ryzen 5, 8GB RAM, 512GB SSD.</p><p><strong>Budget 8-12 Juta:</strong> Laptop mid-range untuk kebutuhan desain atau programming. Spek lebih tinggi, layar lebih baik, baterai lebih awet.</p><p><strong>Tips penting:</strong> Prioritaskan SSD, minimal 8GB RAM, dan prosesor minimal i5/Ryzen 5 generasi terbaru. Jangan tergiur harga murah dengan spek rendah — performa akan mengecewakan dalam 1-2 tahun.</p>',
                'icon': '🎓',
                'is_featured': False,
                'order': 2,
            },
