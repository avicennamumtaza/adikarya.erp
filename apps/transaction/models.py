...
class TransactionDetail(models.Model):
    header = models.ForeignKey(TransactionHeader, on_delete=models.CASCADE, related_name='items')
    product = models.ForeignKey('inventory.Product', on_delete=models.PROTECT)
    qty = models.PositiveIntegerField()
    price_at_trx = models.DecimalField(max_digits=12, decimal_places=2)
    cost_at_trx = models.DecimalField(max_digits=12, decimal_places=2) # Snapshot HPP tepat pada saat itu